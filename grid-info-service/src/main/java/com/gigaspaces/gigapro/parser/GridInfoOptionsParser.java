...
public class GridInfoOptionsParser {
    ...
    private static final Set<XapOption> GRID_INFO_SERVICE_OPTIONS = EnumSet.of(LOOKUP_GROUPS, LOOKUP_LOCATORS, USERNAME, RMI_SERVER_HOSTNAME);
    ...
    private static final String WAIT_TIMEOUT_OPT_DESCRIPTION = "Wait timeout";
    ...
    public static GridInfoOptions parse(String[] args) throws ParseException {
        ...
        xapConfigCLI.addOption(GSA_COUNT_OPT_NAME, GSA_COUNT_OPT, GSA_COUNT_LONG_OPT, GSA_COUNT_OPT_DESCRIPTION, 1, Integer.class);
        xapConfigCLI.addOption(WAIT_TIMEOUT_OPT_NAME, WAIT_TIMEOUT_OPT, WAIT_TIMEOUT_LONG_OPT, WAIT_TIMEOUT_OPT_DESCRIPTION, 1, Long.class);
    ...
    private static int getGsaCount(CommandLine cl) {
        try {
            Integer result = (Integer) cl.getParsedOptionValue(GSA_COUNT_OPT_NAME);
            if (result == null) {
    ...
    private static long getWaitTimeout(CommandLine cl) {
        try {
            Long result = (Long) cl.getParsedOptionValue(WAIT_TIMEOUT_OPT_NAME);
            if (result == null) {
                LOG.info("No wait timeout provided. The default timeout = {} seconds will be used", DEFAULT_TIMEOUT);
                return DEFAULT_TIMEOUT;
            }
            return result;
        } catch (ParseException e) {
            LOG.warn("Invalid wait timeout provided. The default timeout = {} seconds will be used.", DEFAULT_TIMEOUT, e);
            return DEFAULT_TIMEOUT;
        }
    }
}
