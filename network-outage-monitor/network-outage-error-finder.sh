...
export LOG_FILE_REGEXES='^.*/[0-9]{4}-[0-9]{2}-[0-9]{2}~[0-9]{2}.[0-9]{2}-gigaspaces-[a-z]{3}.*\.log;^.*/[a-z]{3}.*\.log'
# The base dir with logs
export BASE_LOG_DIR=/gigaspaces/scripts/logs/

# Split LOG_FILE_REGEXES into the array
REGEXES_ARR=(${LOG_FILE_REGEXES//;/ })
...
UNIQUE_FILES=(`for file in "${FOUND_FILES_ARRAY[@]}"; do echo "$file" ; done | sort -du`)
