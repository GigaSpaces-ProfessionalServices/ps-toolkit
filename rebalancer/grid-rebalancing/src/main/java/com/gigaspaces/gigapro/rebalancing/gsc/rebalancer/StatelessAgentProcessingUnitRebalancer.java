...
/**
 * @author Denys_Novikov Date: 05.04.2018
 */
public class StatelessAgentProcessingUnitRebalancer extends AgentProcessingUnitRebalancer {
    ...
    protected void doRebalancingBetweenContainers(GridServiceAgent gsa) {       
            for (int i = 0; i < MAX_REBALANCING_COUNT; i++) {
                ...
                Map<GridServiceAgent, List<GridServiceContainer>> emptyContainersMap =
                    buildEmptyContainersMap(Collections.singletonList(gsa));
    ...
    private boolean quickSwapRestartBetweenContainers(Map<GridServiceContainer, Integer> lowInstances,
            Map<GridServiceContainer, Integer> highInstances, Map<GridServiceAgent, List<GridServiceContainer>> emptyContainersMap) {
        boolean moved = false;
    ...
    private GridServiceContainer getEmptyContainer(Map<GridServiceAgent, List<GridServiceContainer>> emptyContainersMap,
            GridServiceContainer lowInstanceGSC) {
        GridServiceContainer gsc;
        List<GridServiceContainer> emptyGscs =
            emptyContainersMap.values().stream().flatMap(Collection::stream).collect(Collectors.toList());
